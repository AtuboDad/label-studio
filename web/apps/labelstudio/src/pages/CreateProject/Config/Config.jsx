  ...
  return (
    <div className={configClass.elem("labels")}>
      <form className={configClass.elem("add-labels")} action="">
        <h4>{tagname === "选项" ? "新增选项" : "新增标签"}</h4>
        <span>换行作为分隔符，可添加多个标签</span>
      ...
      <div className={configClass.elem("current-labels")}>
        <h3>
          {tagname === "选项" ? "新增选项" : "新增标签"} ({control.children.length})
        </h3>
        <ul>
      ...
      <select className="border" onChange={selectValue} value={isManual ? "-" : value}>
        {columns?.map((column) => (
          <option key={column} value={column}>
            {column === DEFAULT_COLUMN ? "<导入文件>" : `$${column}`}
          </option>
        ))}
        {!columns?.length && <option value={value}>{"<导入文件>"}</option>}
  ...
  return (
    <div className={configClass.elem("object")}>
      ...
      {template.objects.length > 1 && columns?.length > 0 && columns.length < template.objects.length && (
        <p className={configClass.elem("object-error")}>此模板需要比现在更多的数据</p>
      )}
      {columns?.length === 0 && (
  ...
  return (
    <div className={configClass}>
      <div className={configClass.elem("container")}>
        <h1>标签界面{hasChanges ? " *" : ""}</h1>
        <header>
          <Button type="button" data-leave={true} onClick={onBrowse} size="compact">
            浏览模板
          </Button>
          <ToggleItems items={{ code: "代码", visual: "预览" }} active={configure} onSelect={onSelect} />
        </header>

    ...
    <div className={importClass}>
      ...
      <header className="flex gap-4">
        <form className={`${importClass.elem("url-form")} inline-flex`} method="POST" onSubmit={onLoadURL}>
          <Input placeholder="数据URL" name="url" ref={urlRef} style={{ height: 40 }} />
        ...
        >
          ...
          <label>
            <input {...csvProps} value="ts" checked={csvHandling === "ts"} /> 时间序列或全文文件
          </label>
        </div>
        <div className={importClass.elem("status")}>
          {files.uploaded.length ? `${files.uploaded.length} 个上传文件` : ""}
        </div>
      </header>
              ...
              {projectConfigured && ff.isFF(ff.FF_SAMPLE_DATASETS) ? (
                <SimpleCard title="预期输入预览" className="w-[650px] h-full">
                  {sampleConfig.data ? (
                    ...
                    <div className="w-full pt-4 text-lg text-negative-content">无法加载样例数据</div>
                  ) : null}
                </SimpleCard>

  ...
  observer(({ store, size, hasSelected, ...rest }) => {
    ...
    const invokeAction = (action, destructive) => {
      if (action.dialog) {
        ...
        dialog({
          title: title ? title : destructive ? "删除操作" : "确认操作",
    ...
    const recordTypeLabel = isFFLOPSE3 && store.SDK.type === "DE" ? "记录" : "选中";
    ...
    return (
      ...
      >
        <Button size={size} disabled={!hasSelected} {...rest}>
          {selectedCount > 0 ? `${selectedCount} ${recordTypeLabel}${selectedCount > 1 ? "" : ""}` : "活动"}
